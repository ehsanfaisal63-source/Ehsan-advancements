    ...
    <Card className="w-full max-w-sm bg-background/30 backdrop-blur-lg border-white/10">
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-4">
            <Button variant="outline" onClick={() => handleOAuth('google')} disabled={isProviderLoading !== null || !auth}>
                {isProviderLoading === 'google' ? <Loader2 className="mr-2 animate-spin" /> : <GoogleIcon />}
                Google
            </Button>
            <Button variant="outline" onClick={() => handleOAuth('github')} disabled={isProviderLoading !== null || !auth}>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            />
            <Button type="submit" className="w-full" disabled={isLoading || isProviderLoading !== null || !auth}>
